...
describe("Gizmo Create New Leads", () => {
    ...
    it("It should open an external web page and create new leads", () => {
        ...
        cy.get("#first_name", { timeout: 15000 }).should("be.visible").type("Automation");
        ...
        cy.get("#email").should("be.visible").type("automationcypress@hytechhome.com");
...
it("When users click on Leads functionality link, Users will be redirected to an internal web page with all leads created", () => {
  ...
  cy.get("a[href='https://gizmo.local/user/module/leads']").should("be.visible").click();

  // Wait for potential page load
  cy.wait(15000);

  // Ensure the redirection is correct
  cy.url({ timeout: 20000 }).should("equal", "https://gizmo.local/user/module/leads");
});
...
it("Users must be able to edit existing leads", () => {
  ...
  cy.get(".text.dtfc-fixed-left", {timeout: 50000}).should("be.visible").eq(0).click();  
  
  // Click on one of the elements 
  cy.get(".ownerlookup", {timeout: 60000} ).contains("Nicole Marie").first().click(); 



  // Click on the edit button
  cy.get("#web_edit_url", { timeout: 60000 }).should("be.visible").first().click();
  cy.wait(20000);
...
it("Users must be able to upload files or attachments for existing users", () => {
  ...
  cy.get("a[href='https://gizmo.local/user/module/leads']", {timeout: 10000 }).first().click();
  cy.url({ timeout: 20000 }).should("equal", "https://gizmo.local/user/module/leads");
  ...
  cy.get('.datetime', {timeout: 30000} ).contains('Sep 16, 2022 4:24 PM').click(); 

  // Scroll the Upload File button into view and click it
  cy.get("#attachments-modal", { timeout: 60000 }).scrollIntoView().should('be.visible').click();

  cy.get('[type="button"]', {timeout: 20000 }).first().click({force: true});


  cy.get(".ff_fileupload_dropzone", {timeout: 60000} ).scrollIntoView().should("be.visible").first().click( {force: true} );

  // Ensure the modal containing '#view-comment-in-record' is visible before interacting
  cy.get('#gizmo-comments-modal.modal.fade.text-left', { timeout: 60000 }).should('have.class', 'show').then(($modal) => {
    // Ensure the '#view-comment-in-record' button is visible within the modal
    cy.wrap($modal).find('#view-comment-in-record').should('be.visible').click({force: true});
  });  

// Ensure the modal containing '#view-comment-in-record' is visible before interacting
cy.get('#gizmo-comments-modal.modal.fade.text-left', { timeout: 60000 }).should('exist').then(() => {
  // Ensure the modal is visible by checking its CSS property 'display'
  cy.get('#gizmo-comments-modal.modal.fade.text-left').should('have.css', 'display', 'none');

  // Ensure the '#view-comment-in-record' button is visible within the modal
  cy.get('#view-comment-in-record', { timeout: 60000 }).should('be.visible').click();
});
  ...
  cy.get(".ff_fileupload_preview_image.ff_fileupload_preview_image_has_preview", { timeout: 50000 }).should('contain', fileName); 

  // Click the upload button
  cy.get("#upload", { timeout: 50000 }).should("be.visible").first().click({ force: true });
});
...
it("Users must be able to see preview of the file or attachment uploaded", () => {
  ...
  cy.get("#email").type("jose.mateo@hytechhome.com");
  cy.get("#password").type("Migueldeveloper2424@");
  cy.get(".btn.btn-success.btn-lighten-2.btn-block.btn-sm.waves-effect.waves-float.waves-light").click();
  cy.wait(15000);
  cy.url().should("equal", "https://gizmo.local/user/home");
  ...
  cy.get(".btn.btn-sm.dropdown-toggle.hide-arrow", {timeout: 20000} ).click();
...
it("When users click on file name or attachment name an external link must be opened displaying current file or attachment", () => {
  ...
  // Click on one of the elements 
  cy.get(".ownerlookup", {timeout: 60000} ).contains("Jose Mateo").first().click(); 
...
it("Users must be able to download file or attachment that are uploaded", () => {
   ...
   // Navigate to leads module
   cy.get(".nav-link.menu-toggle").first().click();
   cy.get("a[href='https://gizmo.local/user/module/leads']").first().click({force: true});
   cy.url({ timeout: 20000 }).should("equal", "https://gizmo.local/user/module/leads");
   ...
   cy.get(".btn.btn-sm.dropdown-toggle.hide-arrow", {timeout: 30000 }).click();
...
it('should create a new meeting for an existing lead', () => {
  ...
  cy.get('.btn.btn-sm.btn-success.round.waves-effect.inline-block.btn-new-meeting', { timeout: 30000 })
    .scrollIntoView()
    .should('be.visible')
    .click({ force: true });
  ...
  cy.get('.datetime-picker.field_start_time').should('be.visible').first().click({ force: true });

  cy.get(".form-control").should("be.visible");

  // Retry logic for the calendar visibility
  cy.get(".flatpickr-calendar.hasTime.animate", { timeout: 40000 }) 
    .then(($calendar) => {
  ...
  cy.get(".flatpickr-monthDropdown-months").should("be.visible").contains("June");
